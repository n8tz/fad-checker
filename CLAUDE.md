...
5. Reports EOL frameworks (endoflife.date — Maven **and** npm: jQuery/Angular/AngularJS/Vue/React/Bootstrap), obsolete libs (curated Maven + npm-registry per-version `deprecated` field — authoritative, skips nothing), outdated libs (Maven Central + npm registry `dist-tags.latest`).
