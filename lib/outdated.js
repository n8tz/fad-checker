...
function findEolProduct(dep) {
	// npm packages have no groupId; match on the package name instead. The npm
	// package literally named "angular" is AngularJS 1.x; modern Angular ships
	// as @angular/* scoped packages — hence the separate name vs. scope maps.
	//
	// WebJars (org.webjars*) are client-side JS libraries shipped as Maven
	// artifacts: the artifactId IS the JS library name (e.g. org.webjars:angularjs,
	// org.webjars.npm:angular__core where "__" encodes a scope slash). They must
	// resolve through the same JS-name maps, not the Maven coord maps.
	const g = dep.groupId || "";
	const isWebjar = g === "org.webjars" || g.startsWith("org.webjars.");
	if (dep.ecosystem === "npm" || isWebjar) {
		const name = (dep.artifactId || "").replace(/__/g, "/");
		const byName = EOL_MAPPING.by_npm_name?.[name];
		if (byName) return byName;
		const scopes = Object.keys(EOL_MAPPING.by_npm_scope || {})
			.sort((a, b) => b.length - a.length);
		for (const s of scopes) {
			if (name.startsWith(s)) return EOL_MAPPING.by_npm_scope[s];
		}
		return null;
	}
