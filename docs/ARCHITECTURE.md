...
9. **EOL / Obsolete / Outdated** — `lib/outdated.js` (Maven) + `lib/npm/registry.js` (npm):
   - **EOL**: matches dep coord against `data/eol-mapping.json`, fetches the cycle list from endoflife.date (cached 7d), flags cycles past their EOL date. npm packages **and WebJars** (`org.webjars*` — client-side JS shipped as Maven artifacts, e.g. `org.webjars:angularjs:1.8.3`) match via `by_npm_name` / `by_npm_scope` by JS library name (e.g. npm `angular`/webjar `angularjs` → AngularJS 1.x, `@angular/*`/`angular__core` → Angular, `react`/`react-dom`/`jquery`/`vue`/`bootstrap`).
